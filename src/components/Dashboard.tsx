...
export const Dashboard: React.FC<DashboardProps> = ({
  ...
  return (
    <div className="p-3 sm:p-5 lg:p-6 space-y-5 max-w-[1600px] mx-auto text-slate-900">
      ...
      {/* 0. Hospital Ward & Specialty Category Filter Toolbar */}
      <div className="bg-white p-3.5 sm:p-4 rounded-2xl border border-slate-200 shadow-2xs flex flex-col md:flex-row items-stretch md:items-center justify-between gap-3">
        <div className="flex flex-wrap items-center gap-2 sm:gap-2.5">
          <div className="flex items-center gap-1.5 text-slate-500 mr-1">
            <Filter className="w-4 h-4 text-teal-600 shrink-0" />
            <span className="uppercase tracking-wider text-[10px] font-bold text-slate-500">Filter By:</span>
          </div>

          {/* Ward Dropdown */}
          <div className="flex items-center gap-1.5 bg-slate-50 hover:bg-slate-100 border border-slate-200 rounded-xl px-2.5 py-1.5 transition-colors">
          ...
          {/* Clear Filter Button */}
          {((selectedWard && selectedWard !== 'ALL') || (selectedCategory && selectedCategory !== 'ALL')) && (
            <button
              onClick={onResetFilters}
              className="text-xs text-rose-600 hover:text-rose-700 font-bold bg-rose-50 hover:bg-rose-100 border border-rose-200 px-2.5 py-1.5 rounded-xl transition-all flex items-center gap-1 cursor-pointer shadow-2xs"
            >
        ...
        <div className="flex items-center gap-2 text-xs text-slate-500 justify-between md:justify-end shrink-0">
          <span className="flex items-center gap-1.5 font-medium">
            ...
            <span>
              {(selectedWard && selectedWard !== 'ALL') || (selectedCategory && selectedCategory !== 'ALL') ? (
                <>
                  Showing <strong className="text-slate-900 font-bold">{totalPatients}</strong> of{' '}
                  {allPatientsCount ?? totalPatients} patients
                </>
              ) : (
                <>
                  Total Active Inpatients: <strong className="text-slate-900 font-bold">{totalPatients}</strong>
                </>
              )}
            </span>
          </span>
        ...
        <div className="text-center py-12 bg-white rounded-2xl border border-dashed border-slate-300 p-8 space-y-3">
          ...
          <p className="text-xs text-slate-500 max-w-sm mx-auto">
            {searchQuery
              ...
              ? `No active patients found in ${selectedWard !== 'ALL' ? selectedWard : 'selected ward'}${selectedCategory !== 'ALL' ? ` under ${selectedCategory}` : ''}.`
              : `There are currently no patients under the ${activeFilter} filter.`}
          </p>
          <button
            onClick={() => {
              onResetFilters?.();
